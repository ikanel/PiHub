...
async def initEnvBroadcast():
    ...
    if(settings.environmentBroadcastinterval>0):
...
async def initWebcamBroadcast():
    if(settings.webcamBroadcatInterval>0):
        print("Webcam broadcast has started")
        await webcam.pollWebcam(settings.webcamBroadcatInterval,broadcastWebcamMessage)
